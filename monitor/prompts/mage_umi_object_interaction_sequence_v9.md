...
Describe the interaction as a short numbered sequence in this exact format:

Target object: <object name>
1. <first visible interaction or relevant change>
2. <next visible interaction or relevant change>
3. <next visible interaction or relevant change>
...
- Include only events supported by the video. Do not fill in a standard action sequence.
- Do not infer the intended task, intent, success, failure, or desired final state.
- Do not report timestamps or key frames.
- Use 1 to 8 numbered items. If the target object is never visible, say so directly and do not invent an interaction.
- Output only the requested sequence. Do not output JSON, markdown headings, or additional analysis.
